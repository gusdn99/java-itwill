 ...
 * 		(주의) 원소 타입은 클래스 이름만 사용 가능. 기본 타입을 사용할 수 없음. // ArrayList<int> a; 이거 안됨.
 * 		기본 타입 대신에 wrapper 클래스를 사용.
 * 
 * ArrayList<E>의 특징:
 * 1. 배열을 이용한 리스트 - 값들이 연속된 메모리 공간에 저장.
 * 2. 값의 저장(add), 삭제(remove) 속도가 느림.
 * 3. 검색(get) 속도 빠름.
 * 
 * LinkedList<E>의 특징:
...
public class ListMain01 {
	...
	public static void main(String[] args) {
		// 변수 선언하는 클래스의 <>는 원소타입을 생략할 수 없음.
		ArrayList<Integer> numbers = new ArrayList<>(); // new ArrayList<Integer>();
		// -> 생성자 호출에서는 ArrayList의 원소 타입을 생략할 수 있음.
		
		// ArrayList에 저장된 원소 개수
		System.out.println("size = " + numbers.size()); // size()의 리턴 타입은 int // ArrayList에서는 length()가 없음
		System.out.println(numbers); // 대괄호만 보임
		...
		System.out.println(numbers); // toString() 오버라이드
		
		// ArrayList의 원소 수정
		numbers.set(0, 50); // set(int index, Integer element): 수정할 인덱스 원소 호출, Integer 타입으로 원소 수정.
		System.out.println(numbers);
		
		// ArrayList의 원소 삭제
		numbers.remove(1); // remove(int index): 해당 인덱스의 원소 삭제. int 타입
		System.out.println(numbers);
