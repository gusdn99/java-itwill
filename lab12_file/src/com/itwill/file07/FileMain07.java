 ...
 * 디코딩(decoding): 문자 코드(정수) 그 코드에 해당하는 문자로 변환. (예) 65 -> 'A'
 * 운영체제(OS)의 기본 인코딩 방식:
...
public class FileMain07 {
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputStream in = new FileInputStream(ansiFile); // FileNotFoundException // ansiFile
		
		// 2. 인코딩을 설정한 InputStreamReader(문자 스트림) 객체를 생성.
//		InputStreamReader isr = new InputStreamReader(in, "UTF-8"); 문자들이 깨져서 보임.
		InputStreamReader isr = new InputStreamReader(in, "EUC-KR"); // UnsupportedEncodingException
		
		// 3. 읽기 속도를 빠르게 하기 위해서 버퍼(메모리)를 사용. (선택사항)
		BufferedReader reader = new BufferedReader(isr);
		
		while (true) {
			int read = reader.read(); // 파일에서 1글자를 읽음. // IOException
			if (read == -1) { // EOF
				break;	
			}
			System.out.print((char) read); // 디코딩
		}
		System.out.println();
...
//		FileReader fr = new FileReader(ansiFile, Charset.forName("EUC-KR"));
		FileReader fr = new FileReader(utf8File, Charset.forName("UTF-8")); // utf8File
		
		// 2. 읽기 속도를 빠르게 하기 위해서 버퍼 사용. (선택사항)
