...
public class FileMain02 {
	...
	public static void main(String[] args) throws Exception { // throws Exception
		...
		FileInputStream in = new FileInputStream(origin); // main 메서드에 throws Exception
		FileOutputStream out = new FileOutputStream(dest); // main 메서드에 throws Exception
		
		long start = System.currentTimeMillis(); // 파일 복사 시작 시간 측정 // 리턴 타입이 long
		
		while (true) {
			// (1) read() 메서드 사용:
			// int b = in.read(); // main 메서드에 throws Exception // 파일에서 1바이트 읽음.
			
			// (2) read(byte[] b) 메서드 사용:
			byte[] buffer = new byte[4 * 1024]; // 4,096바이트(4KB) 크기의 빈 배열 생성
			int b = in.read(buffer); // 몇 바이트를 읽었는지
			...
			// out.write(buffer); // -> 바이트 배열의 내용을 그대로 파일에 씀. 4KB를 파일에 씀. // 몇 바이트를 읽었는지
