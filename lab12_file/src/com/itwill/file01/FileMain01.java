 ...
 * 	(3) FOS 객체를 닫음(close). 리소스 해제.
 */

public class FileMain01 {

	public static void main(String[] args) {
		String origin = "data/hello.txt"; // 원본 파일(읽을 파일) 경로, 이름. // 폴더이름/파일이름
		String destination = "data/hello_copy.txt"; // 복사할 파일 경로, 이름.
		
		FileInputStream in = null;
		FileOutputStream out = null;
		try {
			in = new FileInputStream(origin); // FIS 객체 생성 // FileNotFoundException. try-catch 만들어줘야 함.
			out = new FileOutputStream(destination); // FOS 객체 생성 // FileNotFoundException. try-catch 만들어줘야 함.
			while (true) {
				int read = in.read(); // 파일에서 1바이트 읽음. // IOException. try-catch 만들어줘야 함.
				if (read == -1) { // 파일 끝(EOF: End-Of-File)에 도달했을 때
					break;
				}
				out.write(read); // 파일에 읽은 1바이트를 씀. // IOException. try-catch 만들어줘야 함.
//				System.out.println((char) read);
			}
			System.out.println("파일 복사 성공");
		} catch (Exception e) { // IOException의 상위클래스 
			e.printStackTrace();
		} finally {
			try {
				in.close(); // FIS 객체를 닫음(리소스 해제). // IOException. try-catch 만들어줘야 함.
				out.close(); // FOS 객체를 닫음(리소스 해제). // IOException. try-catch 만들어줘야 함.
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
