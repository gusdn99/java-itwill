...
//		calc.calculate((double x, double y) -> { return x / y; });
		result = calc.calculate((x, y) -> x / y); // ((파라미터 1, 2) -> 리턴값);
		System.out.println("result = " + result);
		 ...
		 * - 파라미터 선언에서 변수 타입은 생략 가능. // 예) double
		 * - 람다 표현식 몸통({})에 한 문장만 있는 경우, {}와 세미콜론 생략 가능.(둘다 같이 생략해야 함.)
		 * - 람다 표현식 몸통에 return 문장만 있는 경우에는, 리턴 값만 남김.(return 생략.)
		 * 		(x, y) -> x / y
