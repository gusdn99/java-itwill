 ...
 * - equals(): 객체 동등 비교 메서드.
 * 		힙(heap)에 생성된 객체가 같으면(객체의 주소) true, 다르면 false를 리턴.
 * - hashCode(): 객체의 해시코드(정수) 값을 리턴.
 * 		o. 객체의 주소값으로 해시코드를 만듦.
 * 		o. 해시코드가 만족해야 하는 조건
 * 			1. 같은 객체에서 hashCode() 여러번 호출하더라도 항상 같은 정수가 리턴되어야 함.
 * 			2. equals() 메서드의 결과가 true가 되는 두 객체의 해시코드 값은 같아야 함. // equals() 오버라이드하면, hashCode()도 오버라이드해야 함.
...
//		return "T 타입 인스턴스"; // toString 메서드 오버라이드. 
//	}
}
...
public class InheritanceMain05 {
	...
	public static void main(String[] args) {
		...
		System.out.println(t1.equals(t2)); // 파라미터가 boolean 타입. Object 클래스로부터 상속받음. 객체의 주소값이 다름.
		System.out.println(t1.equals(t3)); // 객체의 주소값이 같음.
		// Object 클래스에서 상속받은 equals() 메서드:
		// 두 객체가 같은지(true), 다른지(false)를 반환하는 메서드. (주소 안의 값)
		// Object에서는 실제로 생성된 객체가 같으면 true를 리턴, 그렇지 않으면 false를 리턴. (주소값)
		
		Object o = new Object();
		System.out.println(o);
		
		Random r = new Random();
		System.out.println(r); // 파라미터가 Object 타입 // -> Random 클래스는 toString() 메서드를 override하지 않음.
		
		Scanner sc = new Scanner(System.in);
		System.out.println(sc); // 파라미터가 Object 타입 // -> Scanner 클래스는 toString() 메서드를 override함.
