...
public class HybridCar extends Car { // Car의 기본생성자가 없어서 아규먼트를 갖는 생성자를 만들어야 함.
	// 필드
	private int battery;
	...
	public HybridCar(int fuel, int speed, int battery) {
		...
		super(fuel, speed); // 아규먼트를 갖는 Car 생성자
...
//		super.drive(); // -> 상위 객체의 재정의 하기 전의 메서드 호출
		System.out.println("하이브리드자동차 운전중(연료: " + getFuel() + ", 속력: " + getSpeed() + ", 배터리: " + battery + ")");
	} 
