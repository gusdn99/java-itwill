 ...
 * 
 * final 수식어
 * - 클래스, 클래스의 멤버(필드, 메서드), 지역 변수에서 사용 가능. // 생성자는 불가
 * - final 의미: 바꿀 수 없는.
 ...
 * 
 * 인스턴스 멤버(필드, 메서드) vs 정적(static) 멤버 // 생성자x
 * 1. 인스턴스 멤버
 * 	(1) static 수식어가 없는 멤버.
 * 	(2) 객체를 생성산 후에 참조 변수를 이용해서 사용하는 멤버.
 * 	(3) 인스턴스 필드는 JRE(Java Runtime Environment)가 사용하는 메모리 영역 중에 힙에 생성.
 * 2. 정적 멤버(필드, 메서드) // 생성자x // 클래스 내부는 보지 말고 우선적으로 메인 메서드부터 시작하자는 의미
 * 	(1) static 수식어가 사용된 멤버.
 * 	(2) 객체를 생성하지 않아도 사용할 수 있는 멤버.
 * 	(3) 클래스 이름을 이용해서 사용하는 멤버. (예) Math.PI, Math.random(), System.in, System.out, ... // 다른 클래스에 같은 이름의 변수가 있을 수도 있어서 클래스 이름.변수로 함.
...
//	private final String message; 파이널 필드인데 초기화를 안해서 에러 뜸.
	private final String message; // 선언할 때 초기화되지 않은 final 필드
	
	// final 필드를 명시적으로 초기화하는 생성자:
	public ModifierMain06(String message) { // 아규먼트를 갖는 생성자 만들고
		this.message = message; // 명시적으로 초기화하면 에러 해결됨.
	}
...
//		n3 = 100;
		...
		ModifierMain06 app = new ModifierMain06("버스 파업"); // 23번 라인의 생성자 (static이 안 붙음)
