 ...
 *  (1) private: 선언된 클래스 안에서만 접근할 수 있는(보이는) 멤버. 다른 클래스에서 접근 불가(안 보임)
 *  (2) (package): 수식어를 사용하지 않은 경우. 같은 패키지에 있는 클래스에서 접근할 수 있는(보이는) 멤버. 기본(default)
 *  (3) protected: 같은 패키지에 있거나 또는 상속하는 클래스에서 접근할 수 있는(보이는) 멤버.
 *  (4) public: 어디서든 접근할 수 있는(보이는) 멤버. 같은 프로젝트 안의 다른 패키지에서도 접근 가능.
 */
...
//		ex.a = 100; // 값을 변경할 수 x
		...
		ex.b = 200;
		System.out.println("b = " + ex.b);
		ex.info(); // 메서드는 public이라서 ex.a도 보임 // 여기서도 ex.a 값을 변경하지는 x 
